...
app.post("/", async (c) => {
  ...
  if (body.tags) input.tags = typeof body.tags === "string" ? JSON.parse(body.tags) : body.tags;
...
app.put("/:id", async (c) => {
  ...
  if (body.tags !== undefined) input.tags = typeof body.tags === "string" ? JSON.parse(body.tags) : body.tags;
