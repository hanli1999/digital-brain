...
export default function ToolsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { name: string; description: string; category: string; url: string }) =>
      fetch("/api/tools", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }).then((r) => r.json()),
    onSuccess: () => {
      ...
      {tools.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {tools.map((tool) => (
            <div
              key={tool.id}
              className="p-4 rounded-lg border hover:shadow-sm cursor-pointer transition-shadow"
              onClick={() => setSelectedId(tool.id)}
            >
              <div className="flex items-start justify-between mb-2">
                <h3 className="font-medium text-sm">{tool.name}</h3>
                <Badge variant="secondary" className={categoryColors[tool.category] || categoryColors.other}>
                  {tool.category}
                </Badge>
              </div>
          ...
          <div className="space-y-4 text-sm">
            ...
            {selected.url && (
              <p>
                ...
                <a href={selected.url} target="_blank" rel="noreferrer" className="text-blue-500 underline">{selected.url}</a>
              </p>
            )}
