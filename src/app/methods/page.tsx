...
export default function MethodsPage() {
  ...
  const { data: methods = [], isLoading } = useQuery<Method[]>({
    queryKey: ["methods"],
    queryFn: () => fetch("/api/methods").then((r) => r.json()),
  });

  const createMutation = useMutation({
    mutationFn: (data: { title: string; content: string; tags: string }) =>
      fetch("/api/methods", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const allTags = Array.from(new Set(methods.flatMap((m) => {
        ...
        <Dialog open={showNew} onOpenChange={setShowNew}>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-3">
              ...
              <Button
                onClick={() => createMutation.mutate({ title, content, tags: JSON.stringify(tags.split(",").map((t) => t.trim()).filter(Boolean)) })}
      ...
      {filtered.length === 0 ? (
        ...
        <DataTable
          columns={[
            ...
            { key: "tags", header: "标签", cell: (m) => {
              try {
                return (JSON.parse(m.tags) as string[]).map((t: string) => (
                  <span key={t} className="text-xs bg-muted px-1.5 py-0.5 rounded mr-1">{t}</span>
                ));
              } catch { return null; }
            }},
